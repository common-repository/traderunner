<?php
/**
 * Plugin Name: traderunner
 * Description: Connect your WooCommerce to Trade Me
 * Version: 3.13
 * Author: Trade Me
 * Author URI: https://trademe.co.nz
 */
...
if ((get_option('active_plugins') && in_array('woocommerce/woocommerce.php', apply_filters('active_plugins', get_option('active_plugins')))) ||
    ...
    if (!class_exists('TradeRunnerAdmin')) {

        class TradeRunnerAdmin {
            const baseUrl = 'https://traderunner.omnivore.com.au/wooCommerce/connect';
            const flavour = 'traderunner';
            const flavourLabel = 'Trade Runner';
            const keyName = 'CityBeach Trade Runner';
    ...
    if (!class_exists('TradeRunner')) {

        class TradeRunner {
            protected $tag = 'traderunner';
            protected $name = 'Trade Runner';
